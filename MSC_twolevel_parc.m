function MSC_twolevel_parc(iK,iPart,iRep)
...
cK=sK(iK);
load(sprintf('MSC_twolevel_eigen/K%d_part%d_rep%d.mat',cK,iPart,iRep));
...
% save to .mat file
time=toc/3600;
save(sprintf('MSC_twolevel_parc/K%d_part%d_rep%d.mat',cK,iPart,iRep),'img_parc','K','time');
fprintf('Time to do group mean parcellation: %0.2f hours. \n',time);

% save to .nii file
file_mask='graymatter.nii';
file_out=sprintf('MSC_twolevel_parc/K%d_part%d_rep%d.nii',cK,iPart,iRep);
