function MSC_mean_parc(iK,iPart,iRep)
% Group level parcellation by mean MSC.
% 2016-5-28 17:07:18
...
label=Ncut_kway(EV,cK); % parcellation results
tmp=zeros(siz);
tmp(msk_gray)=label;
img_parc=tmp;
...
% save to .mat file
time=toc/3600;
save(sprintf('MSC_mean_parc/K%d_part%d_rep%d.mat',cK,iPart,iRep),'img_parc','K','time');
fprintf('Time to do group mean parcellation: %0.2f hours. \n',time);

% save to .nii file
file_mask='graymatter.nii';
file_out=sprintf('MSC_mean_parc/K%d_part%d_rep%d.nii',cK,iPart,iRep);
