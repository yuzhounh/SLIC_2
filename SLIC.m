function [label,time]=SLIC(img_gray,cN)
% The SLIC algorithm for resting-state fMRI data. 
% Input:
%     img_gray, each row is a voxel, each column is a feature
%     cN, cluster number
% Output:
%     label, the parcellation results
...
% fix the parameter m to be one
m=1;

img_gray=img_gray(:,end-cN+1:end); % the feature number equals the cluster number
img_gray=parc_norm(img_gray); % normalize to unit length
